...
default_args={
    'owner': 'ibrahim',
    'start_date': dt.datetime(2022, 5, 28),
    'retries': 1,
...
def CSV_to_JSON(ti):
    df = ti.xcom_pull(task_ids='Read_from_DB')
    json_list = df.to_dict(orient='records')
    return json_list.copy()

def JSON_to_Mongo(ti):
...
#starting_point >> installing_dep >> psqlTOdf >> csvTOjson >> ending_point